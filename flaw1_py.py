...
def clean(text):
  return (clean_text(re.sub("(\([^)]*\))", '',(str(text)[12:-26]).replace("\\n", " ")))).lower()


lst = []
for entry in ds_train:
  lst = lst + word_tokenize(clean(entry['summary'])+" "+clean(entry['text'])+" "+clean(entry['title']))
...
vectors_dict = {}
with open('/content/drive/My Drive/glove/glove.6B.50d.txt', 'r') as GloVe:
  i = 0
  for line in GloVe:
    values = line.split()
    word = values[0]
...
x_num_of_words = vocabcount #???
y_num_of_words = vocabcount #?
